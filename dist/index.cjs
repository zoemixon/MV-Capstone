...
const MoleculeViewer = ({ molecules, onDeleteMolecule, onSceneReady }) => {
  ...
  const densityCloudContainerRef = react.useRef();
  const densityCloudRendererRef = react.useRef();
  const densityCloudSceneRef = react.useRef();
  const densityCloudCameraRef = react.useRef();
  const densityCloudObjectRef = react.useRef();
  ...
  react.useEffect(() => {
    ...
    if (!densityCloudRendererRef.current) {
      const renderer2 = new THREE__namespace.WebGLRenderer({ alpha: true });
    ...
    if (!densityCloudSceneRef.current) {
      densityCloudSceneRef.current = new THREE__namespace.Scene();
    }
    if (!densityCloudCameraRef.current) {
      densityCloudCameraRef.current = new THREE__namespace.PerspectiveCamera(
        75,
    ...
    function renderSingleMolecule(molecule, sceneData, moleculeIdx) {
      ...
      atoms.forEach((atom, index) => {
        ...
        const sphere = new THREE__namespace.Mesh(
          new THREE__namespace.SphereGeometry(radius, 32, 32),
          new THREE__namespace.MeshPhongMaterial({ color })
        );
        ...
        atomRadiusInput.oninput = () => {
          ...
          sphere.geometry = new THREE__namespace.SphereGeometry(newRadius, 32, 32);
        };
        ...
        radiusInput.oninput = () => {
          ...
          atomMeshes.forEach((mesh, i) => {
            if (atoms[i].elem === elem) {
              ...
              mesh.geometry = new THREE__namespace.SphereGeometry(newRadius, 32, 32);
            }
          });
        };
      ...
      bonds.forEach((bond, index) => {
        ...
        const bondMaterial = new THREE__namespace.MeshPhongMaterial({ color: "#AAAAAA" });
        const start = atom1.position;
        const end = atom2.position;
        const distance = start.distanceTo(end);
        const bondMesh = new THREE__namespace.Mesh(
          new THREE__namespace.CylinderGeometry(0.05, 0.05, distance, 8),
          bondMaterial
        );
        bondMesh.position.copy(start).lerp(end, 0.5);
        const direction = new THREE__namespace.Vector3().subVectors(end, start).normalize();
        bondMesh.setRotationFromQuaternion(
          new THREE__namespace.Quaternion().setFromUnitVectors(new THREE__namespace.Vector3(0, 1, 0), direction)
        );
  ...
  react.useEffect(() => {
    if (scene) {
      ...
      if (hasDensityCloud && isWhite) {
        scene.background = new THREE__namespace.Color("#000000");
      } else {
        scene.background = new THREE__namespace.Color(backgroundColor);
      }
    }
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { children: [
    ...
    /* @__PURE__ */ jsxRuntime.jsxs("div", { style: { margin: "auto" }, children: [
      ...
      /* @__PURE__ */ jsxRuntime.jsx("div", { ref: densityCloudContainerRef, style: { position: "absolute", top: 0, left: 0, width: "1100px", height: "600px", pointerEvents: "none", zIndex: 20 } })
    ] })
  ] });
};
...
function renderDensityCloud(scene, densityData, dimensions, positiveThreshold = 1e-3, negativeThreshold = 1e-3) {
  ...
  const cloud = new THREE__namespace.Points(geometry, material);
  scene.add(cloud);
...
const DensityCloudControls = ({ scene }) => {
  const [currentPositiveThreshold, setCurrentPositiveThreshold] = react.useState(1e-3);
  const [currentNegativeThreshold, setCurrentNegativeThreshold] = react.useState(1e-3);
  ...
  return /* @__PURE__ */ jsxRuntime.jsxs("div", { className: "label-section", children: [
    /* @__PURE__ */ jsxRuntime.jsx("button", { onClick: toggleDensityCloud, children: "Toggle Density Cloud" }),
...
exports.DensityCloudControls = DensityCloudControls;
exports.FileParser = FileParser;
exports.MoleculeViewer = MoleculeViewer;
